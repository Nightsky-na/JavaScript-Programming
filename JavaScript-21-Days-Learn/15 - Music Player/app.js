(() => {
    ...
    function onClick() {
        // Check is it already paused!!!
        if (audioElem.paused) {
            audioElem.play();  
            playBtnElem.className = 'pause'; // playBtnElem.setAttribute('class',`pause`);
        } else {
            audioElem.pause();
            playBtnElem.className = 'play'; // Change classsss 
        }
    }

    function onTimeUpdate(){
        // console.log(audioElem.currentTime); // ได้เลขออกมา แต่ต้องformat ก่อนนนน
        startTimeElem.innerHTML = getDuration(audioElem.currentTime);   // แก้ใน Html 
        progressBarElem.value = audioElem.currentTime;
    }
    
    function getDuration(time){
        const minutes = Math.floor(time / 60 % 60).toString();
        const seconds = Math.floor(time%60).toString().padStart(2,'0'); // อยากได้ 2 ตน. ถ้าเป็นหลักเดียวอยากนำด้วย0
    ...
    function run(){
        // console.log('Hello');
        playBtnElem.addEventListener('click', onClick); // ทำอะไร เกิดอะไร
